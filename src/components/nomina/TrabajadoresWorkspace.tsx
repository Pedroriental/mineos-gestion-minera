...
import type { Personal, PerfilCompensacion } from '@/lib/types';

export type TrabajadoresWorkspaceProps = {
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error('[TrabajadoresWorkspace] Caught client render error:', error, errorInfo);
  }
      ...
      return (
        <div className="flex h-[70vh] w-full flex-col items-center justify-center p-6 text-center">
          <div className="max-w-md rounded-2xl border border-red-500/30 bg-neutral-900/90 p-6 shadow-2xl backdrop-blur-sm">
            ...
            <h2 className="text-lg font-bold text-white mb-2">Error al cargar la base de trabajadores</h2>
            <p className="text-xs text-neutral-400 mb-4">
              Ocurrió un problema al inicializar la tabla de trabajadores. Puedes recargar o restablecer la vista.
            </p>
...
const DynamicTrabajadoresClient = dynamic(
  () => import('@/components/nomina/TrabajadoresRegistryClient'),
  {
    ssr: false,
    loading: () => (
      <div className="flex h-[70vh] w-full flex-col items-center justify-center gap-3">
        <Loader2 className="h-8 w-8 animate-spin text-amber-500" />
        <span className="text-sm font-medium text-neutral-400">Cargando base de trabajadores...</span>
      </div>
    ),
  },
);

export default function TrabajadoresWorkspace(props: TrabajadoresWorkspaceProps) {
  return (
    <TrabajadoresErrorBoundary>
      <DynamicTrabajadoresClient {...props} />
    </TrabajadoresErrorBoundary>
  );
}
