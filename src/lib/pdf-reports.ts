...
export function downloadVoladurasPDF(data: ReporteVoladura[], dateLabel?: string) {
  ...
  autoTable(doc, {
    ...
    head: [[
      'Fecha', 'Turno', 'Mina', 'Vertical', 'Responsable',
      'Huecos', 'Detalle huecos', 'Chupis', 'Detalle chupis',
      'N° Disp.', 'Hora',
      'Fósf.', 'Espag.', 'Vit.E', 'Trenza', 'Arroz',
      'Estado',
    ]],
    body: data.map((d) => {
      const huecos = getHuecosTableDisplay(d);
      const chupis = getChupisTableDisplay(d);
      return [
        d.fecha,
        ...
        d.vertical_disparo || '—',
        d.responsable || '—',
        huecos.total > 0 ? huecos.total : '—',
        formatPerforacionDetallePdf(huecos.lineas),
        chupis.total > 0 ? chupis.total : '—',
        formatPerforacionDetallePdf(chupis.lineas),
        d.numero_disparo || '—',
        d.hora_disparo || '—',
        d.fosforos_lp || '—',
        d.espaguetis || '—',
    ...
    columnStyles: {
      ...
      14: { cellWidth: 11 },
      15: { cellWidth: 11 },
      16: { cellWidth: 12, fontStyle: 'bold' },
    },
  });
