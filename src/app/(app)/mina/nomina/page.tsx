...
// ── Config de área ──────────────────────────────────────────────────────────
const PAGE_AREA: Personal['area'] = 'mina';
const PAGE_TITLE = 'Nómina Mina';
const PAGE_DEFAULT_FORM_AREA: Personal['area'] = 'mina';
...
// ══════════════════════════════════════════════════════════════════════════════
// PÁGINA PRINCIPAL — NÓMINA MINA
// ══════════════════════════════════════════════════════════════════════════════
export default function NominaMinaPage() {
  const { user } = useAuth();
  ...
  const handleProcesarNomina = async () => {
    ...
    const pagos = data.map((p) => ({
      ...
      observaciones: `Nómina Mina ${fmtDate(weekRange.inicio)} al ${fmtDate(weekRange.fin)}`,
    ...
    if (catRow) {
      const { data: gastoRow } = await supabase.from('gastos').insert({
        ...
        descripcion: `Nómina Mina ${fmtDate(weekRange.inicio)} al ${fmtDate(weekRange.fin)} — ${data.length} trabajadores`,
        monto: totalNomina,
        proveedor: 'Nómina interna',
        notas: 'Procesado automáticamente desde módulo Nómina Mina.',
        registrado_por: user?.id,
    ...
    try {
      if (importTab === 'excel') {
        ...
        if (employees.length === 0) {
          setParseError('No se detectaron empleados de Mina. Verifica el formato.');
        } else {
        ...
        const employees = all.filter(e => e.area === PAGE_AREA);
        if (employees.length === 0) {
          setParseError('No se detectaron empleados de Mina. Verifica que el PDF tenga texto extraíble.');
        } else {
          setParsedEmps(employees);
        }
      }
  ...
  const handleImportConfirm = async () => {
    ...
    // Desactivar trabajadores actuales de esta área para reemplazarlos
      ...
      {semanaActual === undefined ? null : !semanaActualProcesada ? (
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 px-5 py-3.5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-emerald-300 font-medium">Nómina Mina de esta semana procesada</p>
      ...
      {currentViewLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 md:hidden">
            ...
            {filtered.length === 0 && (
              <EmptyState icon={<Users className="w-8 h-8" />} title="Sin trabajadores de Mina" description={search ? 'No coincide ningún resultado.' : 'Registra el primer trabajador.'} action={canEdit && !search ? { label: 'Agregar primer trabajador', onClick: openNew } : undefined} />
            )}
          </div>
          ...
          <div className="table-container hidden md:block">
            <table className="data-table">
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={7} className="py-0"><EmptyState icon={<Users className="w-8 h-8" />} title="Sin trabajadores de Mina" description={search ? 'No coincide ningún resultado.' : 'Registra el primer trabajador.'} action={canEdit && !search ? { label: 'Agregar primer trabajador', onClick: openNew } : undefined} /></td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {showBorrarModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setShowBorrarModal(false)}>
          <div className="relative w-full max-w-sm bg-zinc-950 border border-red-500/30 rounded-2xl shadow-2xl p-6 text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 className="text-xl font-bold text-white mb-2">¿Borrar todo el personal de Mina?</h2>
          ...
          <div className="relative w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-semibold text-white/90 flex items-center gap-2">
                <RefreshCw className="w-4 h-4 text-amber-400" /> Procesar Nómina Mina
              </h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => { setShowModal(false); setFormError(null); }}>
          <div className="relative w-full max-w-2xl bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl p-8 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-white/90">{editItem ? 'Editar Trabajador' : 'Nuevo Trabajador — Mina'}</h2>
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-zinc-800 shrink-0">
              <div>
                <h2 className="text-lg font-semibold text-white/90 flex items-center gap-2">
                  <Upload className="w-5 h-5 text-amber-400" /> Importar Nómina Mina
                </h2>
                <p className="text-xs text-white/35 mt-0.5">Solo se importarán trabajadores del área Mina</p>
              </div>
              ...
              {importResult ? (
                <div className="flex flex-col items-center justify-center gap-4 py-6 text-center">
                  ...
                  <div className="w-full max-w-sm space-y-3">
                    ...
                    {detectedWeek.inicio && (
                      <p className="text-xs text-amber-400/70 text-center">📅 Semana detectada automáticamente del archivo</p>
                    )}
                  </div>
                  <div className="flex gap-3 mt-1">
                    <button onClick={closeImport} className="btn-secondary">Solo importar datos</button>
                    <button onClick={() => { closeImport(); setShowProcesarModal(true); }} disabled={!canEdit} className="btn-primary">
