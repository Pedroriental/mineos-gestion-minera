...
// ── Config de área ──────────────────────────────────────────────────────────
const PAGE_AREA: Personal['area'] = 'planta';
const PAGE_TITLE = 'Nómina Molino';
const PAGE_DEFAULT_FORM_AREA: Personal['area'] = 'planta';
...
// ══════════════════════════════════════════════════════════════════════════════
// PÁGINA PRINCIPAL — NÓMINA MOLINO
// ══════════════════════════════════════════════════════════════════════════════
export default function NominaPlantaPage() {
  const { user } = useAuth();
  ...
  const [form, setForm] = useState({
    ...
    nombre_completo: '',
    cargo: '',
    area: PAGE_DEFAULT_FORM_AREA as Personal['area'],
    salario_base: '',
    telefono: '',
    notas: '',
  ...
  const resetForm = () => {
    setForm({ cedula: '', nombre_completo: '', cargo: '', area: PAGE_DEFAULT_FORM_AREA, salario_base: '', telefono: '', notas: '', fecha_ingreso: new Date().toISOString().split('T')[0] });
    setEditItem(null);
  };
  ...
  const handleBorrarTodoPersonal = () => { if (data.length === 0) return; setShowBorrarModal(true); };

  const ejecutarBorradoTodo = async () => {
    setBorrandoTodo(true);
    const { error } = await supabase.from('personal').delete().eq('activo', true).eq('area', PAGE_AREA);
    setBorrandoTodo(false);
    if (error) {
      if (error.message.includes('foreign key constraint')) {
        alert('❌ No se puede borrar el personal porque hay una nómina procesada que depende de ellos.\n\nPor favor, revierte la semana en el Historial antes de borrar los trabajadores.');
      } else {
  ...
  const handleProcesarNomina = async () => {
    ...
    const pagos = data.map((p) => ({
      ...
      observaciones: `Nómina Molino ${fmtDate(weekRange.inicio)} al ${fmtDate(weekRange.fin)}`,
    ...
    if (catRow) {
      const { data: gastoRow } = await supabase.from('gastos').insert({
        ...
        descripcion: `Nómina Molino ${fmtDate(weekRange.inicio)} al ${fmtDate(weekRange.fin)} — ${data.length} trabajadores`,
        monto: totalNomina,
        proveedor: 'Nómina interna',
        notas: 'Procesado automáticamente desde módulo Nómina Molino.',
        registrado_por: user?.id,
  ...
  const handleRevertirSemana = async (sem: NominaSemana) => {
    ...
    if (sem.gasto_id) await supabase.from('gastos').delete().eq('id', sem.gasto_id);
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      if (importTab === 'excel') {
        ...
        const employees = all.filter(e => e.area === PAGE_AREA);
        if (employees.length === 0) {
          setParseError('No se detectaron empleados del Molino. Verifica el formato.');
        } else {
        ...
        const employees = all.filter(e => e.area === PAGE_AREA);
        if (employees.length === 0) {
          setParseError('No se detectaron empleados del Molino. Verifica que el PDF tenga texto extraíble.');
        } else {
          setParsedEmps(employees);
        }
      }
  ...
  const handleImportConfirm = async () => {
    ...
    await supabase.from('personal').update({ activo: false }).eq('activo', true).eq('area', PAGE_AREA);
  ...
  const resetImport = () => { setParsedEmps([]); setParseError(null); setImportResult(null); setCurrentFileName(''); setDetectedWeek({ inicio: null, fin: null }); };
  ...
  const filtered = currentViewData.filter((p) => p.nombre_completo.toLowerCase().includes(search.toLowerCase()) || p.cedula.includes(search));
    ...
    <div className="space-y-6">
      ...
      {semanaActual === undefined ? null : !semanaActualProcesada ? (
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 px-5 py-3.5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-emerald-300 font-medium">Nómina Molino de esta semana procesada</p>
        ...
        <div className="flex items-center gap-2">
          {isHistoricalView ? (
            ...
            <>
              ...
              <button onClick={() => { setWeekRange({ inicio: getWeekStart(), fin: getWeekEnd() }); setShowProcesarModal(true); }} disabled={!canEdit || data.length === 0} className="btn-secondary disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2">
                <RefreshCw className="w-4 h-4" /><span className="hidden sm:inline">Procesar</span>
              </button>
              {canEdit && data.length > 0 && (
                <button onClick={handleBorrarTodoPersonal} disabled={borrandoTodo} className="flex items-center gap-1.5 text-xs text-red-400/70 hover:text-red-400 transition-colors disabled:opacity-40 border border-red-500/20 hover:border-red-400/40 bg-red-500/5 rounded-xl px-3 py-2 font-medium" title="Eliminar todos los trabajadores">
      ...
      {todasSemanas.length > 0 && (
        <div className="card-glass overflow-hidden">
          <button onClick={() => setShowHistorial(!showHistorial)} className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-white/[0.03] transition-colors">
                      ...
                      <tr key={sem.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/30 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-white/75 font-medium">{fmtDate(sem.semana_inicio)}</span>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button onClick={() => handleRevertirSemana(sem)} disabled={!canEdit || revertiendoId === sem.id} className="flex items-center gap-1.5 text-xs text-red-400/60 hover:text-red-400 transition-colors disabled:opacity-40 border border-red-500/20 hover:border-red-400/40 bg-red-500/5 rounded-lg px-2.5 py-1.5 whitespace-nowrap">
      ...
      {currentViewLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-4 md:hidden">
            ...
            {filtered.length === 0 && (
              <EmptyState icon={<Users className="w-8 h-8" />} title="Sin trabajadores del Molino" description={search ? 'No coincide ningún resultado.' : 'Registra el primer trabajador.'} action={canEdit && !search ? { label: 'Agregar primer trabajador', onClick: openNew } : undefined} />
            )}
          </div>
          <div className="table-container hidden md:block">
            <table className="data-table">
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={7} className="py-0"><EmptyState icon={<Users className="w-8 h-8" />} title="Sin trabajadores del Molino" description={search ? 'No coincide ningún resultado.' : 'Registra el primer trabajador.'} action={canEdit && !search ? { label: 'Agregar primer trabajador', onClick: openNew } : undefined} /></td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {/* ── Modales (Borrar, Procesar, Agregar/Editar, Importar) ─────────── */}
      {showBorrarModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setShowBorrarModal(false)}>
          <div className="relative w-full max-w-sm bg-zinc-950 border border-red-500/30 rounded-2xl shadow-2xl p-6 text-center" onClick={(e) => e.stopPropagation()}>
            <div className="mx-auto w-12 h-12 bg-red-500/10 rounded-full flex items-center justify-center mb-4 border border-red-500/20"><Trash2 className="w-6 h-6 text-red-500" /></div>
            <h2 className="text-xl font-bold text-white mb-2">¿Borrar todo el personal del Molino?</h2>
            <p className="text-sm text-white/60 mb-6">Estás a punto de eliminar <strong className="text-white">{data.length}</strong> trabajadores de forma <strong className="text-red-400">permanente</strong>.<br /><br />Esta acción no se puede deshacer.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowBorrarModal(false)} disabled={borrandoTodo} className="flex-1 px-4 py-2 bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl transition-colors font-medium border border-zinc-800 disabled:opacity-50">Cancelar</button>
              <button onClick={ejecutarBorradoTodo} disabled={borrandoTodo} className="flex-1 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl transition-colors font-medium flex items-center justify-center disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setShowProcesarModal(false)}>
          <div className="relative w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-semibold text-white/90 flex items-center gap-2"><RefreshCw className="w-4 h-4 text-amber-400" /> Procesar Nómina Molino</h2>
            ...
            <div className="space-y-4">
              <WeekPicker label="Período de esta nómina" value={weekRange} onChange={setWeekRange} />
              <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4 space-y-2.5">
                <div className="flex justify-between text-sm"><span className="text-white/50">Trabajadores activos</span><span className="text-white/80 font-medium">{data.length}</span></div>
                <div className="flex justify-between text-sm"><span className="text-white/50">Total a registrar</span><span className="text-amber-400 font-bold text-base">{fmtMoney(totalSemana)}</span></div>
                <div className="border-t border-zinc-800 pt-2.5 space-y-1">
                  <p className="text-xs text-white/30">Al confirmar se registrará:</p>
                  <p className="text-xs text-white/45">✓ {data.length} pagos en historial</p>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={handleProcesarNomina} disabled={procesando || !weekRange.inicio || !weekRange.fin} className="btn-primary flex-1 disabled:opacity-40">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => { setShowModal(false); setFormError(null); }}>
          <div className="relative w-full max-w-2xl bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl p-8 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-white/90">{editItem ? 'Editar Trabajador' : 'Nuevo Trabajador — Molino'}</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={closeImport}>
          <div className="relative w-full max-w-3xl bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-zinc-800 shrink-0">
              <div>
                <h2 className="text-lg font-semibold text-white/90 flex items-center gap-2"><Upload className="w-5 h-5 text-amber-400" /> Importar Nómina Molino</h2>
                <p className="text-xs text-white/35 mt-0.5">Solo se importarán trabajadores del área Molino</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              {!importResult && (
                <div className="flex gap-1 bg-zinc-900 rounded-xl p-1 w-fit">
                  <button onClick={() => { setImportTab('excel'); resetImport(); }} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${importTab === 'excel' ? 'bg-zinc-800 text-white shadow' : 'text-white/40 hover:text-white/70'}`}><FileSpreadsheet className="w-4 h-4 text-emerald-400" /> Excel</button>
                  <button onClick={() => { setImportTab('pdf'); resetImport(); }} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${importTab === 'pdf' ? 'bg-zinc-800 text-white shadow' : 'text-white/40 hover:text-white/70'}`}><FileText className="w-4 h-4 text-red-400" /> PDF</button>
                </div>
              )}
              {importResult ? (
                <div className="flex flex-col items-center justify-center gap-4 py-6 text-center">
                  <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center"><CheckCircle2 className="w-8 h-8 text-emerald-400" /></div>
                  <div>
                  ...
                  <div className="w-full max-w-sm space-y-3">
                    <WeekPicker label="Procesar nómina para la semana:" value={weekRange} onChange={setWeekRange} />
                    {detectedWeek.inicio && <p className="text-xs text-amber-400/70 text-center">📅 Semana detectada automáticamente del archivo</p>}
                  </div>
                  <div className="flex gap-3 mt-1">
                    <button onClick={closeImport} className="btn-secondary">Solo importar datos</button>
                    <button onClick={() => { closeImport(); setShowProcesarModal(true); }} disabled={!canEdit} className="btn-primary"><DollarSign className="w-4 h-4" /> Registrar en Contabilidad</button>
                  </div>
                </div>
              ) : parsedEmps.length > 0 ? (
                <>
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm"><span className="text-white/40">Archivo:</span><span className="text-white/70 font-medium truncate max-w-[160px]">{currentFileName}</span></div>
                  ...
                  <WeekPicker label={detectedWeek.inicio ? '📅 Semana detectada (puedes corregirla)' : 'Semana de esta nómina'} value={weekRange} onChange={setWeekRange} />
                ...
                <>
                  {importTab === 'excel' ? (
                    <><DropZone accept=".xlsx,.xls,.ods,.csv" label="Arrastra tu archivo Excel aquí (.xlsx, .xls, .ods)" onFile={handleFile} disabled={parsing} /><div className="flex items-start gap-2 text-xs text-white/30"><ChevronRight className="w-3.5 h-3.5 shrink-0 mt-0.5" /><span>Se detecta automáticamente la semana y los salarios.</span></div></>
                  ) : (
                    <><DropZone accept=".pdf" label="Arrastra tu archivo PDF de nómina aquí" onFile={handleFile} disabled={parsing} /><div className="flex items-start gap-2 text-xs text-white/30"><ChevronRight className="w-3.5 h-3.5 shrink-0 mt-0.5" /><span>El PDF debe tener texto extraíble (no escaneado).</span></div></>
                  )}
                  {parsing && <div className="flex items-center justify-center gap-3 py-6 text-amber-400"><Loader2 className="w-5 h-5 animate-spin" /><span className="text-sm">Procesando archivo...</span></div>}
                  {parseError && <div className="flex items-start gap-2 px-3 py-3 rounded-xl bg-red-500/10 border border-red-500/20"><AlertCircle className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" /><span className="text-sm text-red-400">{parseError}</span></div>}
                </>
              )}
            </div>
            {!importResult && parsedEmps.length > 0 && (
              <div className="flex justify-between items-center gap-3 px-6 py-4 border-t border-zinc-800 shrink-0">
                <p className="text-sm text-white/40">Importar <span className="text-white/70 font-semibold">{validCount}</span> empleados{invalidCount > 0 && <span className="text-amber-400/70"> ({invalidCount} omitidos)</span>}</p>
                <div className="flex gap-3">
