/**
 * Resumen Ejecutivo — Server Component
 *
 * Arquitectura:
 *   page.tsx (Server)  →  lee searchParams.period
 *                      →  llama 3 RPCs en paralelo (PostgreSQL)
 *                      →  renderiza HTML en el servidor
 *   PeriodSelector.tsx (Client)  →  cambia la URL, triggerea re-fetch
 *   loading.tsx        →  skeleton automático mientras resuelve
 *
 * No hay useEffect, no hay useState, no hay fetch del lado cliente.
 */
...
import {
  ...
  Factory, AlertTriangle, Pickaxe, Scale, Target, Calendar,
} from 'lucide-react';
...
// ── Tipos para Next.js 16 App Router ─────────────────────────
...
export default async function ResumenEjecutivoPage({ searchParams }: PageProps) {
  // Leer el período desde la URL (default: 30 días)
  const { period: periodParam } = await searchParams;
  const period = ([7, 15, 30, 90].includes(Number(periodParam))
    ? Number(periodParam)
    : 30) as PeriodoDias;

  // 3 RPCs en paralelo — PostgreSQL hace todo el cálculo pesado
  ...
  // ── Area chart (SVG nativo, sin dependencias) ───────────────
  const chartNode = (() => {
    ...
    const toY = (v: number) =>
      H - PAD - Math.max((v / maxOro) * (H - PAD * 2 - 24), 1);
    const pts = prodDiaria
      .map((d, i) => `${i * W + W / 2},${toY(Number(d.oro_g))}`)
      .join(' ');
    const areaPath = [
      `M 0,${H}`,
  ...
  return (
    <div className="space-y-6 print:space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        ...
        {/* Client island: solo el selector interactivo */}
        <PeriodSelector currentPeriod={String(period)} />
      </div>

      {/* Gold Banner — Resultado financiero principal */}
        ...
        <div className="lg:col-span-2 card-glass rounded-xl p-5 flex flex-col">
          ...
          {gastosCat.length > 0 ? (
                ...
                return (
                  <div key={cat.categoria}>
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-xs text-white/65 font-medium truncate pr-3">
                        {cat.categoria}
                      </span>
            ...
            <div className="flex-1 flex items-center justify-center">
              <p className="text-sm text-white/30 text-center">Sin gastos registrados</p>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          ...
          <div key={card.title} className="card-glass rounded-xl p-5"
            style={{ borderTop: `2px solid ${card.accent}40` }}>
