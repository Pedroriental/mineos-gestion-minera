  ...
  return (
    <html lang="es" suppressHydrationWarning>
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){if(!('serviceWorker' in navigator))return;var h=location.hostname;var dev=h==='localhost'||h==='127.0.0.1'||h==='[::1]';function clearSw(){navigator.serviceWorker.getRegistrations().then(function(regs){regs.forEach(function(r){r.unregister()})});if('caches' in window){caches.keys().then(function(keys){keys.forEach(function(k){caches.delete(k)})})}}if(dev){clearSw();return}function isChunkMsg(m){return m&&(m.indexOf('ChunkLoadError')>=0||m.indexOf('Loading chunk')>=0||m.indexOf('Failed to load chunk')>=0)}window.addEventListener('unhandledrejection',function(ev){var r=ev&&ev.reason;var msg=(r&&r.message)||String(r||'');if(!isChunkMsg(msg))return;var k='mineos-chunk-reload-v1';if(sessionStorage.getItem(k))return;sessionStorage.setItem(k,'1');clearSw().finally(function(){location.reload()})});window.addEventListener('load',function(){navigator.serviceWorker.register('/sw.js?v=6').then(function(r){console.log('SW registered',r.scope)}).catch(function(e){console.warn('SW failed:',e)})})})();`,
          }}
        />
      </head>
